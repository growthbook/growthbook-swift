...
class GrowthBookSDKBuilderTests: XCTestCase {
    ...
    let tempDirectoryURL: URL = URL(filePath: NSTemporaryDirectory()).appendingPathComponent(UUID().uuidString)
//    instance: instance, context: gbContext, featuresFetchResultHandler: featuresFetchResultHandler, logLevel: growthBookBuilderModel.logLevel, networkDispatcher: networkDispatcher, cachingManager: cachingManager
    func testSDKInitializationDefault() throws {
        let sdkInstance = GrowthBookBuilder(
            growthBookBuilderModel: GrowthBookModel.init(
                instance: .init(
                    apiHostURL: URL(string: testApiHost)!,
                    clientKey: testClientKey,
                    payloadType: .ciphered(encryptionKey: testKeyString),
...
//                stickyBucketService: .none
            ),
//            featuresFetchResultHandler: .none,
//            networkDispatcher: GrowthBookNetworkClient(),
            cacheOptions: .init(
                directoryURL: tempDirectoryURL,
                featureCacheFilename: "features.json",
                savedGroupsCacheFilename: "savedGroups.json"
            )
        ).initializer()
    ...
    func testSDKInitializationOverride() throws {
        ...
        let forcedVariations: [String: Int] = [:]
...
//                logLevel: .info,
                isQaMode: true,
                isEnabled: false,
                forcedVariations: JSON(forcedVariations),
//                stickyBucketService: .none
            ),
//            featuresFetchResultHandler: .none,
//            networkDispatcher: GrowthBookNetworkClient(),
        ...
        XCTAssertTrue(sdkInstance.getGBContext().attributes == testAttributes)
        XCTAssertTrue(sdkInstance.getGBContext().forcedVariations == JSON(forcedVariations))
