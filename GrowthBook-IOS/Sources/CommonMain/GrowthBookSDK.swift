...
@objc public class SDKBuilderApp: NSObject, SDKBuilderProtocol {
    ...
    private var logHandler: SDKLogHandler?
    ...
    /// Set log handler for SDK Logger
    public func setLogHandler(_ logHandler: @escaping SDKLogHandler) -> SDKBuilderApp {
...
@objc public class GrowthBookSDK: NSObject, FeaturesFlowDelegate {
    ...
    init(context: Context,
        ...
        if let logHandler = logHandler {
            logger = Logger(label: "GrowthBook-Logger", factory: logHandler)
        }
    ...
    @objc public func refreshCache() {
        ...
        if let hostUrl = gbContext.hostURL {
            apiUrl = hostUrl + Constants.featurePath + gbContext.apiKey
        }
